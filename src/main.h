...
public:
    ...
    unsigned int nTime;
    unsigned int nBits;  // fuhcoin: prime chain target, see prime.cpp
    unsigned int nNonce;

    // fuhcoin: proof-of-work certificate
    ...
    // fuhcoin: header hash does not include prime certificate
    uint256 GetHeaderHash() const
    {
        return Hash(BEGIN(nVersion), END(nNonce));
    }

    // fuhcoin: block hash includes prime certificate
    uint256 GetHash() const
    {
...
public:
    ...
    unsigned int nPrimeChainType;   // fuhcoin: chain type (memory-only)
    unsigned int nPrimeChainLength; // fuhcoin: chain length (memory-only)
    ...
    bool ReadFromDisk(const CDiskBlockPos &pos)
    {
        ...
        // fuhcoin: no proof-of-work check here unlike bitcoin
        // Check the header
        return true;
    }
...
public:
    ...
    uint256 nChainWork;
    unsigned int nWorkTransition; // fuhcoin: work transition ratio (memory-only)

    unsigned int nPrimeChainType;   // fuhcoin: chain type
    unsigned int nPrimeChainLength; // fuhcoin: chain length
    int64 nMoneySupply;             // fuhcoin: money supply
    ...
    {
        // fuhcoin: disabled proof-of-work check for loading block index
...
public:
    uint256 hashPrev;
    uint256 hashBlock; // fuhcoin: persist block hash as well
